...
Plots:
 - Generate a table of axis_x x axis_y plots:
      ...
      cols are different values of psi (inside each plot is p)
...
class PlotPsi:
    ...
    axis_x = ['equity', 'bankruptcies', 'interest_rate', 'bad_debt','asset_i','asset_j', 'prob_bankruptcy','c']
    with_log = ['equity']

    titles_x = ['$log(E)$', 'bankruptcy', '$i_r$', '$B$','$asset_i$','$asset_j$','$prob_{bank}$','$c$']
    ...
    def plot(self, working_dir:str, input_experiments: list, output:str, correlations_files:list=None,
        ...
        for i, item_i in enumerate(self.axis_x):
            ...
            for j, item_j in enumerate(self.axis_y):
                if not os.path.isdir(self.working_dir + '/' + item_j) and item_j:
