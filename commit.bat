...
if %errorlevel%==0 (
 powershell -command "(Get-Content interbank.py)   -replace 'import interbank_lenderchange as lc','' -replace 'interbank_lenderchange\.',''  -replace 'lc\.','' -replace 'import interbank_lenderchange as lc','' | Out-File -encoding Default inter.py"
