...
\node[textnode, right=of links] (links_text) {
    Using Barabási–Albert graph of $m$ edges or Erdös-Rényi graph with probability of attachment $p_a$.};
...
\node[textnode, right=of shock1_text] (shock2_text) {
    \begin{tabular}{l|l}
        ...
        $L^i_t$  & $E^i_t$ \\
        $R^i_t\uparrow$ if $\Delta D>0$  & $d$ (demand of loan $\Delta D<0$)
    \end{tabular}  };
...
\node[textnode, right=of shock2__text1] (shock2__text2) {
   \begin{tabular}{l|l}
        ...
        $C^i_t \uparrow$ if $\Delta D>0$&  $D^i_t$\\
        $L^i_t$  & $E^i_t$ \\
        $R^i_t \uparrow$ if $\Delta D>0$  & $d$ (demand of loan if $\Delta D<0$)
   \end{tabular}  };
...
\node[textnode, below=0.1cm of repayments_text7] (repayments_text8) { \\
    \begin{tabular}{l|l}
        \multicolumn{1}{c|}{\textbf{Assets}} &  \multicolumn{1}{c}{\textbf{Liabilities}} \\ \hline
        $C^i_t \uparrow$ &  $D^i_t$\\
        $L^i_t$  & $E^i_t$ \\
