...
\node[block, below=of init] (shock1) {
...
\node[block, below=4cm of repayments] (links) {
  \texttt{setup\_links()} };
...
\node[statistics, anchor=north east, at=(loans.south)] (loans_stats1) {
  asset\_i asset\_j \\ psi\_lenders psi\_loans \\
  active\_lenders/borrowers};

\node[statistics, right=0cm of loans_stats1] (loans_stats2) {
  interest\_rate  leverage\\
  equity\_lenders\\
  systemic\_leverage
};
...
\node[statistics, below=0cm of repayments_stats1] (repayments_stats2) {
  policy deposits \\
  potential\_credit\_channels \\
  reserves
};
\node[statistics, anchor=north east, at=(links.south)] (links_stats) {
...
\node[textnode, right=of init] (init_text) { $bank.B=0$};
...
\node[textnode, right=0.3cm of links] (links_text1) {
...
\node[textnode, right=0.3cm of links_text1] (links_text2) {
  Rate for next period of time: \\
  $r_t^{i,j} = \frac{\chi A_t^i - \phi A_t^j - (1 - p_t^j)(\xi A_t^j - c_t^{i,j})}{p_t^j c_t^{i,j}} (1 + \psi)$
 };
\node[textnode, below=0.3cm of links_text2] (links_text3) {
  Profits: \\
  $\mu_t^i =  \eta_t \frac{C_t^i}{C^{max}_t}  + (1 - \eta_t) \frac{r^{min}_t}{r_t^i} $
 };
\draw[->] (links) -- (links_text1);
\draw[->, shorten >=1pt, shorten <=1pt] (links_text1) -- (links_text2);
\draw[->] (links_text2) -- (links_text3);
...
\draw[->] (repayments) -- (links);
\draw[->] (sim.east) -- (finish);
\draw[->] (links.south) |- ++(0,-1) -- ++(-3,0) |- (sim.west);
