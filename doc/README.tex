...
model.set_policy_recommendation(eta=0.5)
    ...
    \item \texttt{exp\_runner\_no\_concurrent.py}: Another sub-prototype that avoids the use of parallelism.
    \item \texttt{exp\_runner\_comparer.py}: A derivation of the former prototype though to compare the evolution with $p_a$ (probability of attachment in an Erdos-Renyi graph) in the $x$ axis and other parameters across the $y$ axis.
...
\begin{itemize}
    ...
    \item \texttt{interbank.py --save result.gdt --stats\_market --detail\_banks 5,7}: Save the results in \texttt{result.gdt}, a second file \texttt{resultb.gdt} with the results for only banks and times participating really in the loans market is generated, and also a third file \texttt{result\_detailed.gdt} with the concrete statistics for banks 5 and 7.
    With \texttt{--detail\_times 10,12} all specific details for all banks in times 10 and 12 are present in this third file.
    \item \texttt{interbank.py --fast}: Use a fast mechanism to execute the model (useful when running big models or repetitions).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{incrD1}: Amount of ($ \Delta D$) for the bank in first shock
    \item \textbf{incrD2}: Amount of ($ \Delta D$) for the bank in second shock
    \item \textbf{incrD}: Sum of $ \Delta D$ for both shocks
    \item \textbf{ir}: Interest rate $r$ of the bank who are potential lenders ($\Delta D>0$)
    \item \textbf{ir\_effective}: Interest rate $r$ of the bank who have really loans with borrowers
    ...
    \item \textbf{maxE}: $E_{\max}$ of the system
    ...
    \item \textbf{sum\_loans}: Amount borrowed by the bank
...
\textbf{incrD,incrD1,incrD2} &  float &   $\sum$  &  $\sum$  &  &  $\checkmark$  \\
\textbf{ir}                &  float &  $\overline{x}/0$ & $\overline{x} / nan $  & $\checkmark$ &  \\
\textbf{ir\_effective}     &  float &  $\overline{x}/0$ & $\overline{x} / nan $  & $\checkmark$ &  \\
...
\textbf{maxE}        &  float &  $\checkmark$ & $\checkmark$ &  & \\
...
\textbf{sum\_loans}        & float &  $ \sum$ &  $ \sum$ & $\checkmark$ &  \\
