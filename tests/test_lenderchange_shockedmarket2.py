...
class RestrictedMarketTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.model.config.lender_change = interbank_lenderchange.determine_algorithm("ShockedMarket2")
    ...
    def test_values_after_execution(self):
        ...
        self.assertIsInstance(self.model.config.lender_change, interbank_lenderchange.ShockedMarket2)

        for t in range(T):
            self.assertEqual(self.graphs[t].is_directed(), False)
            # only one link for each node: only one lender, so 0 or 1 the value:
            for i in range(BANKS):
                self.assertLessEqual(len(self.graphs[t].edges(i)), BANKS)
